...
"""
 ...
 If anything breaks, the hearbeat will
    switch to alternating red/orange (if one of the subprocesses died)
    be solid red (if something went wrong in the main thread)
    freeze in its blue/green heartbeat (if the main thread was terminated and not allowed to exit cleanly)
"""
class AcousticJoystickController:
  ...
  def run(self):
    # Create state that will be shared among the processes
    manager = Manager()
    ...
    try:
      ...
      while True:
        ...
        # note that the LEDs behaved weirdly if more than one process was controlling them
    ...
    # Something went wrong: kill any remaining processes
